...
public class Kost {
    ...
    public Kost(String nama, String alamat, String deskripsi, int jumlahKamar, int hargaPerBulan) {
        ...
        if (nama == null || nama.trim().isEmpty()) {
            throw new IllegalArgumentException("Nama kosan tidak boleh kosong");
        }
        if (alamat == null || alamat.trim().isEmpty()) {
            throw new IllegalArgumentException("Alamat kosan tidak boleh kosong");
        }
        if (deskripsi == null || deskripsi.trim().isEmpty()) {
            throw new IllegalArgumentException("Deskripsi kosan tidak boleh kosong");
        }

        // handle integer value yang tidak diperbolehkan
        if (jumlahKamar < 0) {
            throw new IllegalArgumentException("Jumlah kamar tidak boleh negatif");
        }
        if (hargaPerBulan <= 0) {
            throw new IllegalArgumentException("Harga per bulan harus lebih besar dari 0");
        }
