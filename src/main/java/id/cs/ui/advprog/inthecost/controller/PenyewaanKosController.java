...
public class PenyewaanKosController {
    ...
    @PutMapping
    public PenyewaanKos update(@RequestBody PenyewaanKosRequestDto dto) {
        PenyewaanKos existing = service.findById(dto.getId());

        existing.setNamaLengkap(dto.getNamaLengkap());
        existing.setNomorTelepon(dto.getNomorTelepon());
        existing.setTanggalCheckIn(dto.getTanggalCheckIn());
        existing.setDurasiBulan(dto.getDurasiBulan());

        if (dto.getKostId() != null) {
            Kost kost = kosRepository.findById(dto.getKostId())
                    .orElseThrow(() -> new RuntimeException("Kost not found"));
            existing.setKos(kost);
        }
